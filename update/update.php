<?php
if (!class_exists('WP_Memory_Login_Update')) 
{
	...
	{		
		...
		public $url_version = "lifeguard-version";
		...
		function __construct() 
		{
			...
			register_activation_hook(__DIR__ ."/".$this->main_file, array( $this,'activate_cron_accions_lifeguard'));
				
			//ACTION TO DO WHEN PLUGINS DEACTIVATE
			register_deactivation_hook(__DIR__ ."/".$this->main_file, array( $this,'desactivate_cron_accions_lifeguard'));
		}
		...
		public function activate_cron_accions_lifeguard() 
		{
			//DEFINE ACTION TO DAILY CRON ACTION
			if (! wp_next_scheduled ( 'auto_update_lifeguard' )) 
				wp_schedule_event(time(), 'daily', 'auto_update_lifeguard');	
			
			//ADD ACTION FOR UPDATE CRON ACTION
			add_action('auto_update_lifeguard', array( $this,'auto_update_plugin'));
		}
		//ACTION TO DO ON DEACTIVE PLUGIN
		public function desactivate_cron_accions_lifeguard() 
		{
			wp_clear_scheduled_hook('auto_update_lifeguard');
		}
